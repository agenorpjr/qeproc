...
import { getAllOrders, getOrdersByProd, getProductByName, getProducts } from '@/lib/orders/getOrderData';
import { redirect, useRouter } from 'next/navigation';
import { useSession } from 'next-auth/react';
import dayjs from 'dayjs';
import { ActionIcon, Box, Button, Card, Divider, Flex, Grid, Group, Modal, Stack, Title, Text, TextInput, Anchor, Combobox, useCombobox } from '@mantine/core';
...
export default function SearchByProdPage() {
  ...
  async function searchOrders() {
    const pId = await getProductByName(selectedProduct)
    const op = await getOrdersByProd(pId)
  ...
  return (
    <>
      <Flex justify='flex-start'><Title order={2} mb={20} className={classes.title} >Busca Requisições por Produto</Title></Flex>
      <Grid mb={20}>
        <Grid.Col span={{ base: 12, xs: 8 }}>
          ...
          >
            <Combobox.Target>
              <TextInput
                label="Produto"
                placeholder="Selecione o produto para pesquisar"
        ...
        <Grid.Col span={{ base: 12, xs: 4 }} >
          <Flex align='flex-end' h='100%'>
            <Button variant='outline' color='indigo' onClick={searchOrders}>Pesquisar</Button>
          </Flex>
        </Grid.Col>
      </Grid>
